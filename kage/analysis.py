    ...
    def analyse(self):
        for truth in self.truth_variants:
            ...
            if self.predicted_variants.has_variant(truth):
                if (
                    self.predicted_variants.get(truth).get_genotype()
                    == truth.get_genotype()
                ):
                    self._true_positive[truth.type] += 1

            if not self.predicted_variants.has_variant(truth) or (
                self.predicted_variants.has_variant(truth)
                and self.predicted_variants.get(truth).get_genotype() == "0/0"
            ):
        ...
        for variant in self.predicted_variants:
            ...
            if self.truth_regions.is_inside_regions(variant.position):
                if (
                    variant.get_genotype() != "0/0"
                    and not self.truth_variants.has_variant(variant)
                ):
                    self._false_positive[variant.type] += 1

        logging.info("--- REPORT ---")
        for type in ["SNP", "DELETION", "INSERTION"]:
            precision = self._true_positive[type] / (
                self._true_positive[type] + self._false_positive[type]
            )
            recall = self._true_positive[type] / (
                self._true_positive[type] + self._false_negative[type]
            )
            logging.info(
                "%s. Recall: %.4f, Precision: %.4f" % (type, recall, precision)
            )
...
class GenotypeDebugger:
    def __init__(
        ...
        self.false_negatives_with_zero_counts_and_other_truth_variant_close = (
            defaultdict(int)
        )
        self.false_negatives_with_zero_counts_and_other_truth_variant_close_only_one_side = defaultdict(
            int
        )
    ...
    def print_report(self):
        for type in ["SNP", "DELETION", "INSERTION", "SUBSTITUTION"]:
            ...
            logging.info(
                "N false negatives with zero counts: %d"
                % self.false_negatives_with_zero_counts[type]
            )
            logging.info(
                "N false negatives with zero counts and truth variant close: %d (only one side: %d)"
                % (
                    self.false_negatives_with_zero_counts_and_other_truth_variant_close[
                        type
                    ],
                    self.false_negatives_with_zero_counts_and_other_truth_variant_close_only_one_side[
                        type
                    ],
                )
            )
            ...
            if self.n_predicted_variants[type] > 0:
                logging.info(
                    "Precision (all genotype types): %.4f"
                    % (self.n_correct_genotypes[type] / self.n_predicted_variants[type])
                )

    def print_info_about_variant(
        self, reference_node, variant_node, variant, variant_id
    ):
        ...
        logging.warning("")
        assert (
            self.predicted_genotypes.get(variant).genotype != ""
        ), self.predicted_genotypes.get(variant).vcf_line
        logging.warning(
            "Predicted: %s. Correct is %s. Variant id: %d"
            % (
                self.predicted_genotypes.get(variant),
                self.truth_genotypes.get(variant),
                variant_id,
            )
        )
        ...
        logging.warning(
            "Node counts on ref/alt %d/%d: %d/%d"
            % (
                reference_node,
                variant_node,
                self.node_counts.node_counts[reference_node],
        ...
        logging.info(
            "Model on ref node: %s"
            % self.node_count_model.describe_node(reference_node)
        )
        logging.info(
            "Model on alt node: %s" % self.node_count_model.describe_node(variant_node)
        )
        ...
        logging.info(
            "Predicted/true most similar: %s / %s"
            % (
                self.predicted_genotypes[most_similar],
                self.truth_genotypes.get(most_similar_variant),
            )
        )
        ...
        logging.info(
            "Node counts on helper nodes %d/%d: %d/%d"
            % (helper_ref, helper_alt, helper_count_ref, helper_count_alt)
        )
        logging.info(
            "Model on helper ref: %s" % self.node_count_model.describe_node(helper_ref)
        )
        logging.info(
            "Model on helper alt: %s" % self.node_count_model.describe_node(helper_alt)
        )
        ...
        logging.info(
            "Count    probs most similar (log): %s" % self.count_probs[most_similar]
        )
        ...
        logging.warning(
            "Kmers on ref/variant node %d/%d: %s / %s"
            % (reference_node, variant_node, reference_kmers, variant_kmers)
        )
        logging.warning(
            "Kmer sequences on variant node:   %s"
            % [(hash, kmer_hash_to_sequence(hash, 31)) for hash in variant_kmers]
        )
        logging.warning(
            "Kmer sequences on reference node: %s"
            % [(hash, kmer_hash_to_sequence(hash, 31)) for hash in reference_kmers]
        )
    ...
    def analyse_variant(self, reference_node, variant_node, variant, variant_id):
        ...
        if (
            len(self.reverse_kmer_index.get_node_kmers(reference_node)) == 0
            or len(self.reverse_kmer_index.get_node_kmers(variant_node)) == 0
        ):
            self.n_missing_kmers += 1
            logging.warning(
                "Node %d or %d does not have kmers, variant %s! %d missing so far"
                % (reference_node, variant_node, variant, self.n_missing_kmers)
            )
        ...
        if self.predicted_genotypes.has_variant(variant):
            ...
            if self.truth_genotypes.has_variant(variant):
                if self.truth_genotypes.get(variant) == self.predicted_genotypes.get(
                    variant
                ):
        ...
        if (
            ...
            if (
                self.truth_regions.is_inside_regions(variant.position)
                and self.predicted_genotypes.get(variant).genotype == "0|0"
                and self.truth_genotypes.get(variant).genotype != "0|0"
                and self.truth_genotypes.get(variant).genotype
                != self.predicted_genotypes.get(variant).genotype
            ):
                logging.warning("----------------------------")
                ...
                if self.node_counts[variant_node] == 0:
                    ...
                    window = 31
                    if self.truth_genotypes.has_variant_left_of_variant(
                        variant, window
                    ) or self.truth_genotypes.has_variant_right_of_variant(
                        variant, window
                    ):
                        self.false_negatives_with_zero_counts_and_other_truth_variant_close[
                            variant.type
                        ] += 1
                        if not self.truth_genotypes.has_variant_left_of_variant(
                            variant, window
                        ) or not self.truth_genotypes.has_variant_right_of_variant(
                            variant, window
                        ):
                            self.false_negatives_with_zero_counts_and_other_truth_variant_close_only_one_side[
                                variant.type
        ...
        if (
            ...
            and (
                ...
                or (
                    self.truth_genotypes.has_variant(variant)
                    and self.truth_genotypes.get(variant).genotype == "0|0"
                )
            )
        ):
        ...
        if len(reference_kmers.intersection(variant_kmers)) > 0:
            logging.warning("----------")
            logging.warning(
                "Variant and reference node share kmers on variant %s, nodes %d/%d: %s / %s"
                % (
                    variant,
                    reference_node,
                    variant_node,
            ...
            if len(reference_kmers) == 0:
                logging.warning(
                    "Variant %s %d/%d has 0 reference kmers"
                    % (variant, reference_node, variant_node)
                )
            if len(reference_kmers) == 0:
                logging.warning(
                    "Variant %s %d/%d has 0 variant kmers"
                    % (variant, reference_node, variant_node)
                )
            ...
            if variant.type == "DELETION" or variant.type == "INSERTION":
                ...
                if self.predicted_genotypes.has_variant(variant):
                    logging.warning(
                        "Predicted: %s" % self.predicted_genotypes.get(variant)
                    )
                    if (
                        not self.truth_genotypes.has_variant(variant)
                        and self.predicted_genotypes.get(variant).genotype != "0|0"
                        ...
                        self.truth_genotypes.has_variant(variant)
                        and self.predicted_genotypes.get(variant)
                        != self.truth_genotypes.get(variant)
                    ):
    ...
    def analyse_unique_kmers_on_variants(self):
        ...
        logging.info(
            "Reference nodes that are zero in model: %d"
            % self.n_ref_nodes_zero_in_model
        )
        ...
        logging.info(
            "N ambiguous deletions wrongly genotyped: %d"
            % self.n_ambiguous_wrongly_genotyped
        )
